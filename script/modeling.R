...
#setwd("/Users/andersonlu/Desktop/Stanford Courses/EPI 262")
...

{
  ...
  # data <- read_excel("/Users/tanqingfeng/Documents/EPI262/In-class/V1.academy_award_winners.xls")
...
#Figures
{
  # Figure 1: sex & num_win
  ggplot(data, aes(x = sex, fill = factor(num_win))) +
    geom_bar(position = "fill",alpha=0.8) + theme_minimal() +
  ...
  # Figure 4: genre & num_nom
  ggplot(data, aes(x = num_nom, fill = genre)) +
    geom_histogram(alpha = 0.5, position = "identity", bins = 15) +
    theme_minimal() + 
    labs(title = "Histogram for Number of of Nominations by Film Type",x = "Number of Nominations", y = "Number of People",fill = "Film Type") + 
    scale_x_continuous(breaks = seq(min(data$num_nom), max(data$num_nom), by = 1))
  
  # Figure 5: num_4star_films & sex
...
#Table 1
{
  ...
  t2 <- table1(~ time + sex + race + birth_year + birth_country + name_change + genre + films_total + 
...
# The final model for hypothesis 1: adjusting for: birth year, name change, drama, #films, #four star
cox.fit_h1 = coxph(Surv(tstart, tstop, death_tv==1) ~ win_tv + birth_year + name_change + genre + films_total + films_star, data=data_timevary_h1)
summary(cox.fit_h1)
...
# The final model for hypothesis 2: adjusting for birth year, name change, drama, #films, #four star
cox.fit_h2 = coxph(Surv(tstart, tstop, death_tv==1) ~ win_tv + birth_year + name_change + genre + name_change + genre + films_total + films_star , data=data_timevary_h2)
summary(cox.fit_h2)
...
# The final model for hypothesis 3: adjusting for birth year, name change, drama, #films, #four star
cox.fit_h3 = coxph(Surv(tstart, tstop, death_tv==1) ~ nom_tv + birth_year + name_change + genre + name_change + genre + films_total + films_star, data=data_timevary_h3)
summary(cox.fit_h3)
